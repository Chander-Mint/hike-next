                  ...
                  {link.dropdown ? (
                    <>
                      <button
                        ...
                        className={`text-md font-600 capitalize tracking-wide ${pathname.split('?')[0] === link.href.split('?')[0] ||
                            link.dropdown.some(
                        ...
                        <div className="absolute top-full left-0 mt-2 bg-white border rounded-md shadow-lg py-2 w-32">
                          {link.dropdown.map((item) => (
                            <Link
                              key={item.name}
                              href={item.href}
                              className={`block px-4 py-2 text-sm font-semibold capitalize ${selectedTheme === item.name ||
                ...
                <div key={link.name}>
                  {link.dropdown ? (
                    <>
                      <button
                        onClick={toggleHomeDropdown}
                        className={`text-sm font-semibold capitalize tracking-wide ${pathname.split('?')[0] === link.href.split('?')[0] ||
                            link.dropdown.some(
                              (item) => pathname.split('?')[0] === item.href.split('?')[0]
                            )
                            ? 'text-black underline underline-offset-4'
                        ...
                        <div className="flex flex-col items-center space-y-2 mt-2">
                          {link.dropdown.map((item) => (
                            <Link
                              key={item.name}
                              href={item.href}
                              className={`text-sm font-semibold capitalize ${selectedTheme === item.name ||
                                  pathname.split('?')[0] === item.href.split('?')[0]
                                  ? 'text-black underline underline-offset-4'
                                  : 'text-gray-600 hover:text-black'
                                } transition duration-150`}
